...
class ContenedorCarrito{
    ...
    async create(){
        ...
        try{
            ...
            try{
                let fileCarritos = await fs.promises.readFile(this.file, 'utf-8');
                ...
                try{
                    await fs.promises.writeFile(this.file, JSON.stringify([carritoNuevo], null, 2));
                    return {status: "success",message: `Carrito creado. ID: ${carritoNuevo.id}`}
                }catch{
                    return {status: "error",message: "No se pudo generar el carrito"}
                }
    ...
    async addProduct(number, productoAdd){
        try{
            ...
            if(index === -1){
                ...
                let carritoElegido = carritos[index];
                let producto = new Contenedor('../public/txt/productos.txt');
    ...
    async cartDel(idCart){
        try{
            ...
            if(index === -1){
                ...
                await fs.promises.writeFile(this.file, JSON.stringify(nuevoArchivo, null, 2));
    ...
    maxId() {
        let id = 0
        try{
            let fileCarritos = fs.readFileSync(this.file);
            let carritos = JSON.parse(fileCarritos);
